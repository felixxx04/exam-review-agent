...
> 状态：阶段 0 已完成；阶段 1 的 Task 1.1、Task 1.2 已完成实现，Task 1.2 等待用户验收。
>
...
- [ ] 支持课程、考试日期、长期目标和每日可用时间。
- [ ] 支持每次会话临时覆盖可用时长。
- [ ] 所有资料、会话、图谱、测验和掌握度必须属于一个私人课程。
- [ ] 添加跨用户访问负面测试，包括猜测 ID、批量接口和删除接口。

### Task 1.4：用户数据删除与配额

- [ ] 支持删除会话、资料、测验、错题和记忆。
- [ ] 账号注销创建可追踪删除任务，清理关系数据、对象和向量。
- [ ] 默认限制每用户 100 个文件、2 GB；管理员可调整。
- [ ] 删除失败可重试，最终状态对用户可见。
...
阶段 0 已由用户批准并完成。阶段 1 的 Task 1.1 与 Task 1.2 已获准并完成实现；当前停在 Task 1.2 验收门，Task 1.3 尚未批准。

下一步由用户选择：

- `确认 Task 1.2，开始 Task 1.3`
- `修改计划：<需要调整的内容>`
- `暂停实施，保留当前 Task 1.2 结果`
