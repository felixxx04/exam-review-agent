...
- 状态：Accepted
- 日期：2026-08-03
- 实现：Task 1.2、Task 1.3、Task 1.4、Task 2.1 已完成（2026-08-08）
...
- 删除会话、资料、测验、错题和课程记忆时继续按认证用户与课程限定资源；数据库外键级联负责关系数据，Chroma collection 和存储对象由显式清理器处理。
...
- 对象 Key 固定携带可信 `user_id + course_id + material_id`，但只保存在 PostgreSQL；公开资料响应不返回 Key、对象版本、Hash、存储路径或解析内部错误。
- 签名下载 URL 先由当前认证用户检查资料所有权；跨用户或删除中的资料与不存在统一返回 `404`，并且只有被授权调用的响应才包含短期签名 URL。
- 签名下载还要求资料处于 `available` 状态；崩溃预留、删除中和删除完成的资料不能被租户读取或签名。
- 相同内容只在相同用户和课程内检测重复，跨用户/课程不读取、不复用对象。单资料删除、账号注销、预留回收均按存储版本删除同一租户对象。
- 预留回收 CLI 必须显式接收受信任的 `user_id` 并在每个 PostgreSQL 事务绑定 RLS 上下文；它不是可由浏览器或普通 API 参数触发的跨租户维护入口。
- 上传请求的 ASGI 限制器按块计量并在 multipart 解析器读取前拒绝超限内容；它不缓存完整有效请求，端点的用户锁、配额预留与对象写入只有在完整请求已通过限制后才会执行。
- 外部索引前先持久化 chunk ID 意图并设置带随机 attempt ID 的处理租约；资料、课程和账号删除共享用户锁。worker 在跨系统索引边界后必须重新验证该 attempt ID、活跃租约、资料状态和精确 chunk ID，避免恢复或删除接管后旧 worker 越过租户清理边界。正在索引、不确定写入和预留中的资料保持可恢复状态并阻断级联，普通 `deleting` 资料则可在对象服务恢复后安全重试。
