...
class TestReviewEndpoints:
    @pytest.mark.asyncio
    async def test_get_weak_points(self, client_with_db, mistake_repository):
        response = await client_with_db.get("/api/review/weak-points")
    ...
    @pytest.mark.asyncio
    async def test_list_mistakes_returns_review_records(
        self, client_with_db, mistake_repository
    ):
        await mistake_repository.create({
        ...
        response = await client_with_db.get("/api/review/mistakes")
    ...
    @pytest.mark.asyncio
    async def test_list_mistakes_filters_by_status_and_search(
        self, client_with_db, mistake_repository
    ):
        await mistake_repository.create({
            "type": "mistake_records",
    ...
    @pytest.mark.asyncio
    async def test_get_mistake_detail(
        self, client_with_db, mistake_repository
    ):
        await mistake_repository.create({
            "type": "mistake_records",
        ...
        response = await client_with_db.get("/api/review/mistakes/q-detail")
    ...
    @pytest.mark.asyncio
    async def test_update_mistake_correction_and_mastery(
        self, client_with_db, mistake_repository
    ):
        await mistake_repository.create({
            "type": "mistake_records",
    ...
    @pytest.mark.asyncio
    async def test_daily_session_returns_priority_review_items(
        self, client_with_db, mistake_repository
    ):
        await mistake_repository.create({
            "type": "mistake_records",
        ...
        response = await client_with_db.post(
            "/api/review/daily-session", json={"limit": 5}
        )
    ...
    @pytest.mark.asyncio
    async def test_similar_quiz_uses_mistake_context(
        self, client_with_db, mistake_repository
    ):
        await mistake_repository.create({
            "type": "mistake_records",
        ...
        response = await client_with_db.post(
            "/api/review/mistakes/q-similar/similar-quiz"
        )
    ...
    @pytest.mark.asyncio
    async def test_explain_mistake_returns_and_stores_explanation(
        self, client_with_db, mistake_repository
    ):
        await mistake_repository.create({
            "type": "mistake_records",
        ...
        response = await client_with_db.post(
            "/api/review/mistakes/q-explain/explanation"
        )
        ...
        detail = await client_with_db.get("/api/review/mistakes/q-explain")
        assert detail.json()["data"]["explanation"] == data["explanation"]

    @pytest.mark.asyncio
    async def test_update_mistake_sets_review_schedule_and_history(
        self, client_with_db, mistake_repository
    ):
        await mistake_repository.create({
            "type": "mistake_records",
    ...
    @pytest.mark.asyncio
    async def test_export_mistakes_as_markdown_and_csv(
        self, client_with_db, mistake_repository
    ):
        await mistake_repository.create({
            "type": "mistake_records",
        ...
        markdown = await client_with_db.get(
            "/api/review/export", params={"format": "markdown"}
        )
        csv = await client_with_db.get(
            "/api/review/export", params={"format": "csv"}
        )
